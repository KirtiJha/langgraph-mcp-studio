    ...
    <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden hover:border-slate-600/50 transition-all duration-300 group">
        ...
        {tool.inputSchema?.properties && (
          <div className="flex gap-3 text-sm">
            {getRequiredParams().length > 0 && (
              <div className="flex items-center gap-1 px-2 py-1 bg-amber-500/20 text-amber-300 rounded-full">
                <span className="text-xs">{getRequiredParams().length} required</span>
              </div>
            )}
            {getOptionalParams().length > 0 && (
              <div className="flex items-center gap-1 px-2 py-1 bg-slate-600/50 text-slate-400 rounded-full">
                <span className="text-xs">{getOptionalParams().length} optional</span>
              </div>
            )}
          </div>
        )}
      ...
      {showSchema && tool.inputSchema && (
        <div className="border-t border-slate-700/50 p-6 bg-slate-800/30">
          <h4 className="text-sm font-semibold text-white mb-3">Input Schema</h4>
      ...
      {isExpanded && tool.inputSchema?.properties && (
        <div className="border-t border-slate-700/50 p-6 bg-slate-800/30">
          ...
          {getRequiredParams().length > 0 && (
            <div className="mb-4">
              <h5 className="text-xs font-medium text-amber-300 mb-2">Required Parameters</h5>
              <div className="space-y-2">
                {getRequiredParams().map((param) => (
                  <div key={param.name} className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
          ...
          {getOptionalParams().length > 0 && (
            <div>
              <h5 className="text-xs font-medium text-slate-400 mb-2">Optional Parameters</h5>
              <div className="space-y-2">
                {getOptionalParams().map((param) => (
                  <div key={param.name} className="p-3 bg-slate-700/20 border border-slate-700/50 rounded-lg">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-medium text-white">{param.name}</span>
                      <span className="text-xs px-2 py-0.5 bg-slate-600 text-slate-300 rounded">
                        {param.type}
