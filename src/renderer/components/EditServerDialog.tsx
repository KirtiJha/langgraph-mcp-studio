...
const EditServerDialog: React.FC<EditServerDialogProps> = ({
  ...
  const [contextParams, setContextParams] = useState<Array<{ key: string; value: string }>>([]);
  ...
  useEffect(() => {
    if (serverConfig && open) {
      ...
      const contextArray = Object.entries(serverConfig.contextParams || {}).map(([key, value]) => ({
        key,
        value: value || '',
      }));
  ...
  const updateContextParam = (index: number, field: 'key' | 'value', value: string) => {
  ...
  const importFromJson = () => {
    try {
      ...
      if (parsed.mcpServers) {
        ...
        // Update UI state
        const envArray = Object.entries(newConfig.env || {}).map(([key, value]) => ({
          key,
          value: value || '',
        }));
        setEnvVars(envArray);

        const contextArray = Object.entries(newConfig.contextParams || {}).map(([key, value]) => ({
          key,
          value: value || '',
        }));
        setContextParams(contextArray);
        ...
        throw new Error('Invalid configuration format. Expected "mcpServers" property.');
      }
    } catch (error: any) {
        ...
        {tabValue === 0 && (
          ...
          <Stack spacing={3}>
            ...
            <Stack spacing={2}>
              ...
              <FormControl fullWidth required>
                ...
                <Select
                  value={config.type}
                  onChange={(e) => setConfig({ ...config, type: e.target.value as 'stdio' | 'sse' })}
                >
              ...
              {config.type === 'stdio' ? (
                <>
                  ...
                  <TextField
                    ...
                    multiline
                    rows={2}
                    helperText={`Space-separated arguments. Example: ${examples.args || 'mcp-server-time'}`}
                  />
                </>
              ) : (
                <TextField
                  ...
                  fullWidth
                  required
                  helperText={`Example: ${examples.url || 'http://localhost:3000/sse'}`}
                />
              )}
            </Stack>
          </Box>
          ...
          <Box>
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              ...
              For example, set "repo_path" to automatically provide the repository path to git tools.
            </Alert>
                ...
                {contextParams.map((param, index) => (
                  <Box key={index} display="flex" gap={1} alignItems="center">
                    <TextField
                      ...
                      size="small"
                      sx={{ flex: 1 }}
                      helperText={index === 0 ? "e.g., repo_path, api_key" : undefined}
                    />
                    <TextField
                      label="Value"
                      value={param.value}
                      ...
                      size="small"
                      sx={{ flex: 2 }}
                      helperText={index === 0 ? "e.g., /Users/name/project" : undefined}
                    />
                    <IconButton
        ...
        {tabValue === 1 && (
          ...
          <Stack spacing={3}>
            <Box>
              ...
              <Alert severity="info" sx={{ mb: 2 }}>
                Paste a complete MCP server configuration in JSON format, or edit the exported configuration below.
              </Alert>
    ...
    "my-server": {
      ...
      <DialogActions>
        ...
        <Button
          ...
          variant="contained"
          startIcon={<SaveIcon />}
          disabled={!config.name || (!config.command && config.type === 'stdio') || (!config.url && config.type === 'sse')}
        >
